...
use crate::{
    layout::{Layout, Position, Segment, SegmentId, SegmentLink, SegmentRange},
    mask::Mask,
};
...
pub fn single_method_layout(
     ...
     * 3. Use these call jumps (and the 'jumps' generated by plain leads) to split the course
     *    up into a set of (not necessarily mutually exclusive) fragments which have to be rung
     *    consecutively.  At this point, we also add plain leads as links between segments (the DFS
     *    engine can't tell apart calls and plain leads).
    ...
    let segment_ranges = range_starts
        ...
        .map(|&start| {
            ...
            // `range_ends` refers to the rows *before* calls (e.g. lead **ends**), but to fit with
            // `..` we need `end` to refer to the rows **after** calls (e.g. lead **heads**).
            let end = end + 1;
            ...
            SegmentRange {
                ...
                length: if start < end {
                    ...
                    // If the course wraps round the end of the course, then add an extra course
                    // length (before the subtraction to prevent underflow)
                    end + course_len - start
                },
            }
        })
        .collect_vec();
    ...
    let segment_index_by_start = segment_ranges
        .iter()
        .enumerate()
        .map(|(i, r)| (r.start_idx, i))
        .collect::<HashMap<_, _>>();

    // The index of the method is 0, because compositions can only include one method.  If we
    // extend Monument to spliced, then we should set this differently per method.
    ...
    let segments = segment_ranges
        ...
        .map(|range| {
            ...
            let range_end = range.start_idx + range.length;
            ...
            let next_segment_start_index = range_end % course_len;
            // The `- 1` corrects for the fact that call starts refer to lead ends, whereas ranges
            // are half open and therefore their end index refers to the next lead **head**.  We
            // need `+ course_len` here to prevent the subtraction from causing an underflow if
            // `range_end == 0`.
            let pre_call_index = (range_end + course_len - 1) % course_len;

            /* Calculate which calling position this plain lead should be named. */
            let plain_lead_tenor_place = plain_course
                .get_row(pre_call_index)
                .unwrap()
                .place_of(tenor)
                .unwrap();
            ...
            links.extend(
                call_jumps
                    .iter()
                    // Remove any calls that don't start at the end of this range.
                    .filter(|call_jump| call_jump.from_index == pre_call_index)
                    .map(|call_jump| {
            ...
            Segment {
                ...
                // of
                name: String::new(),
                // For the time being, only allow course heads to have the fixed bells in their
                // home positions (i.e. tenors together major would require `1xxxxx78`).  This
                // should probably be relaxed in future to allow for split-tenors compositions
                // (e.g. tenor rotations in Royal).
                course_head_masks: vec![Mask::fix_bells(
                    method.stage(),
                    fixed_bells.iter().copied(),
                )],
                // The start/end nodes (and their truncations) are generated later by
