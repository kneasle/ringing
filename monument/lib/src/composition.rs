...
use crate::query::{Method, MusicType};
...
impl Composition {
    ...
    /// ```text
    /// Block {
    ///     12345678: (<index of Bastow>, 0),
    ///     21436587: (<index of Bastow>, 1),
    ///     21345678: (<index of Bastow>, 2),
    ///     12436587: (<index of Bastow>, 3),
    ///     14263857: (<index of Cambridge>, 0),
    ///     41628375: (<index of Cambridge>, 1),
    ///     14682735: (<index of Cambridge>, 2),
    ///     41867253: (<index of Cambridge>, 3),
    ///     48162735: (<index of Cambridge>, 4),
    ///        ...
    /// }
    /// ```
    ...
    /// Score generated by just the [`MusicType`]s (not including calls, changes of methods, etc.).
    ...
    /// A slice containing the number of [`Row`]s generated for each [`Method`] in the [`Query`].
    /// These are stored in the same order as the [`Method`]s.
    pub fn method_counts(&self) -> &[usize] {
        self.method_counts.as_slice()
    }

    /// A slice containing the number of *instances* of each [`MusicType`] in the [`Query`].  These
    /// are stored in the same order as the [`MusicType`]s.
    pub fn music_counts(&self) -> &[usize] {
        self.music_counts.as_slice()
    }
}
