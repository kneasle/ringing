...
/// The length of a `Chunk` **in one part**.  This and [`TotalLength`] allow the compiler to catch
...
/// A section of a composition with no internal links, uniquely determined by a [`ChunkId`]
// TODO: Remove this and make `RangeFactory` take a constructor function instead?
#[derive(Debug, Clone)]
pub struct ChunkRange {
    pub chunk_id: ChunkId,
    /// The combined length of this chunk in all parts
...
impl ChunkRange {
    ...
    /// Returns the [`Row`]s covered by this [`ChunkRange`] **of the plain course**
    ...
    /// Get the [`RowRange`] covered by this [`ChunkRange`].  If this is a 0-length end chunk, `None`
    /// is returned.
    pub fn range(&self) -> Option<RowRange> {
        match self.chunk_id {
            ChunkId::Standard(StandardChunkId { row_idx, .. }) => {
    ...
    pub fn links(&self) -> &[(LinkIdx, ChunkId, Rotation)] {
        match &self.range_end {
            RangeEnd::End(_) => &[],
...
/// The ways that a [`ChunkRange`] could end.  Chunks either [`End`] the composition with no
/// successors, or the chunk leads to a set of successors without ending the composition.
#[derive(Debug, Clone, Eq, PartialEq, Hash)]
pub enum RangeEnd {
    NotEnd(Vec<(LinkIdx, ChunkId, Rotation)>),
    End(End),
}
...
pub struct RangeFactory<'a> {
    ...
    /// chunk in the graph), but the [`Rotation`]s keep track of information about which part is
    /// being rung.
...
impl<'a> RangeFactory<'a> {
    pub fn new(layout: &'a Layout, part_head: &Row) -> Self {
        ...
        // In order to make chunk expansion easier, we make sure that the ends are given rotations
        // of 0.  This way, the [`ChunkId`]s of chunks which reach part heads will always be made
    ...
    pub fn start_ids(&mut self) -> Vec<(ChunkId, StartIdx, Rotation)> {
        self.layout
            .starts
            .iter()
            .enumerate()
            .map(|(idx, start)| {
                let source_id =
                    StandardChunkId::new(start.course_head.clone(), start.row_idx, true);
                let (equiv_id, rotation) = self.equiv_id(source_id);
                (ChunkId::Standard(equiv_id), StartIdx::new(idx), rotation)
            })
            .collect_vec()
    }

    /// Returns the [`ChunkRange`] which starts at a given [`ChunkId`].  If this [`ChunkRange`] would
    /// never terminate (because no [`Link`]s or ends can be applied to it), then `None` is
    /// returned.
    pub fn gen_range(&mut self, id: &ChunkId) -> Option<ChunkRange> {
        match id {
            ChunkId::Standard(std_id) => self.gen_range_standard(std_id),
            ChunkId::ZeroLengthEnd => Some(ChunkRange::zero_len_end(self.layout.num_methods())),
        }
    }

    /// Returns the [`ChunkRange`], starting at a given [`StandardChunkId`].  If this [`ChunkRange`] would
    /// never terminate (because no [`Link`]s can be applied to it), then `None` is returned.
    fn gen_range_standard(&mut self, id: &StandardChunkId) -> Option<ChunkRange> {
        ...
        match self.ch_equiv_map.get(id.course_head.as_ref()) {
            ...
            None => panic!(
                "Generating chunk {:?} without giving it an equivalence class",
                id
            ),
        }
        // Sanity check that if `id.is_start`, then this `ChunkId` actually corresponds to a start
        if id.is_start {
            let start = self
                .layout
                .starts
            ...
            assert!(
                start.is_some(),
                "`ChunkId` sets `is_start` but has no corresponding start"
            );
        }
        ...
        let mut outgoing_links = Vec::<(LinkIdx, ChunkId, Rotation)>::new();
        ...
        for (link_idx, link) in self.layout.links.iter_enumerated() {
            ...
            // chunk if this link is taken before the end is reached.
            if cmp_to_shortest_len == Ordering::Less {
                outgoing_links.clear();
                shortest_length = Some(length);
            }
            // If this chunk is at least as good as the current best, then add this link to the list
        ...
        for (idx, end) in self.layout.ends.iter_enumerated() {
                ...
                && end.row_idx.block == id.row_idx.block
            {
                ...
                // Thus, each start chunk would generate a 0-length segment corresponding to a
                ...
                let is_improvement = match shortest_length {
                    // If this chunk ends at the same location that a link could be taken, then the
        ...
        // De-duplicate the links (removing pairs of links which link to the same chunk).  We do
        ...
        // 2. Handling 0-length end chunks in spliced.  If a splice is made just before rounds, then
        ...
        //    of `ChunkId`, 0-length ends are defined to be equal, so the de-duplication will
        //    consider all of these to be equivalent and keep only one.
        let mut deduped_links =
            Vec::<(LinkIdx, ChunkId, Rotation)>::with_capacity(outgoing_links.len());
        ...
        // Generate the string for this chunk from the lead labels annotating the block
        ...
        // new Some(ChunkRange), otherwise bubble the `None` value
        ...
        Some(ChunkRange {
            ...
            label,
            chunk_id: ChunkId::Standard(id.clone()),
            range_end: end_idx
                .map(End::Idx) // Zero-length chunks are handled when links are **created**
                .map_or_else(|| RangeEnd::NotEnd(deduped_links), RangeEnd::End),
        })
    }

    /// Gets the [`ChunkId`] of the chunk that would appear after this [`Link`] is applied to a given
    /// course.
    fn id_after(&mut self, link: &Link, source_ch: &Row) -> (ChunkId, Rotation) {
        ...
        let chunk_id = if self.layout.idx_of_end(&equiv_ch, link.to).is_some() {
            // If this leads directly to an end chunk, then it creates a 0-length end
            ChunkId::ZeroLengthEnd
        } else {
            ChunkId::Standard(StandardChunkId {
                course_head: equiv_ch.to_arc(),
                row_idx: link.to,
                is_start: false, // Chunks reached by taking a link can't be start chunks
            })
        };
        (chunk_id, rotation)
    }

    fn equiv_id(&mut self, source_id: StandardChunkId) -> (StandardChunkId, Rotation) {
        let (equiv_ch, rotation) = equiv_ch(
            &source_id.course_head,
            &mut self.ch_equiv_map,
            &self.part_heads,
        );
        let equiv_id = StandardChunkId::new(equiv_ch, source_id.row_idx, source_id.is_start);
        (equiv_id, rotation)
    }
}
...
fn equiv_ch(
    ...
    // If the equivalence class of this chunk has already been generated, then we simply look up the
