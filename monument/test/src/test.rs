...
// these will all be prefixed with `common::PATH_TO_MONUMENT_DIR`.
//
...
fn run_tests() -> anyhow::Result<Outcome> {
    // Equivalent of `-q`.  Tests run so fast that we don't need progress updates
    monument_cli::init_logging(log::LevelFilter::Warn);

    let cases = sources_to_cases(common::collect_sources(common::RunType::Test)?)?;

    // Run the tests.  We run _tests_ in parallel, but _benchmarks_ sequentially
    println!("running {} tests", cases.len());
    let completed_tests: Vec<RunTestCase> = cases.into_par_iter().map(run_and_print_test).collect();
...
impl Outcome {
    fn colored_string(self) -> ColoredString {
        match self {
            Self::Pass => "ok".color(colors::OK),
            Self::Fail => "fail".color(colors::FAIL),
        }
    }
}
...
fn sources_to_cases(sources: Vec<(CaseSource, CaseUse)>) -> anyhow::Result<Vec<UnrunTestCase>> {
    ...
    for (source, state) in sources {
        ...
        let expected_results = match results_value {
            ...
            _ if state == CaseUse::Parse => ExpectedResult::Parsed,
...
fn run_and_print_test(unrun_case: UnrunTestCase) -> RunTestCase {
    let config = Config {
        ...
        // We're running the *tests* in parallel, so each test should be single-threaded
        num_threads: Some(1),
        ..Config::default()
    };
    // Run the test
...
impl CaseOutcome<'_> {
    fn colored_string(self) -> ColoredString {
        match self {
            Self::Parsed => "parsed".color(colors::OK),
            Self::Ignored => "ignored".color(colors::IGNORED),
            Self::Unspecified(_) => "unspecified".color(colors::UNSPECIFIED),
            Self::Specified { expected, actual } if expected == actual => "ok".color(colors::OK),
            Self::Specified { .. } | Self::ParseError(_) => "fail".color(colors::FAIL),
        }
    }
}
