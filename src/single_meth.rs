...
impl Table<Row> {
    ...
    pub fn from_method(
        ...
        let mut music_patterns = MusicPattern::runs_front_or_back(stage, 4, 1.0);
        // music_patterns.extend(MusicPattern::runs_front_or_back(stage, 5, 1.0));
        // music_patterns.extend(MusicPattern::runs_front_or_back(stage, 6, 1.0));
    ...
    pub fn new(
        ...
        // Generate the music tables for each section.  At the same time, track the best possible
        // music density for any course (used to improve pruning).
        let mut best_scores: Vec<f32> = Vec::new();
        let music_tables = ranges
            .iter()
            .map(|r| {
                // TODO: It would be cleaner to use out-of-the-box dynamic dispatch here
                let (music_table, best_music_score) = if r.start < r.end {
                ...
                best_scores.push(best_music_score);
                music_table
            })
            .collect_vec();

        Table {
            heuristic: Heuristic::new(&best_scores, &range_lengths),
...
impl<R: CompRow> engine::Table<R> for Table<R> {
    ...
    #[inline(always)]
    fn music_upper_bound(&self, section: Section, num_rows: usize) -> f32 {
        // We've already upper bounded the most music we can get out of a row (on average), so use
        // this to upper bound the music over any number of rows).
        self.heuristic.evaluate(section, num_rows)
    }
...
#[derive(Debug, Clone)]
struct Heuristic {
    routes_to_course_head: Vec<(usize, f32)>,
    best_course_density: f32,
}

impl Heuristic {
    fn new(scores: &[f32], lengths: &[usize]) -> Self {
        // TODO: Make this handle arbitrary course structures
        Heuristic {
            routes_to_course_head: (0..scores.len())
                .map(|i| (lengths[i..].iter().sum(), scores[i..].iter().sum()))
                .collect_vec(),
            best_course_density: scores.iter().sum::<f32>() / lengths.iter().sum::<usize>() as f32,
        }
    }

    #[inline(always)]
    fn evaluate(&self, section: Section, num_row_left: usize) -> f32 {
        let (rows_to_course_head, score_to_course_head) = self.routes_to_course_head[section.ind];
        score_to_course_head
            + (num_row_left - rows_to_course_head) as f32 * self.best_course_density
    }
}
